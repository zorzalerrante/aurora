define("matta", ["d3"], function(d3) {
    ...
    matta.prepare_graph = function(graph) {
        ...
        // graph attributes
        if (graph.hasOwnProperty('graph') && graph.graph !== null) {
            console.log('graph graph', graph.graph);
            for (var key in graph.graph) {
                if (graph.graph.hasOwnProperty(key)) {
                    graph[key] = graph.graph[key];
                }
            }
        }
